...
#[test]
fn invalid_aff4_is_reported_as_decode_error() {
    // Bare ZIP magic with no AFF4 metadata is recognized as AFF4 by the sniffer
    // but is not a valid container — it must surface a loud typed decode error
    // (naming AFF4), never be misparsed as a disk or silently ignored.
